...
export function parsePeriodDate(period) {
  ...
  // "01/01/2024" or "1/1/2024" — grab the first MM/DD/YYYY occurrence
  const mdyMatch = s.match(/(\d{1,2})\/(\d{1,2})\/(\d{4})/)
  if (mdyMatch) {
    return _localDate(mdyMatch[3], mdyMatch[1].padStart(2, '0'), mdyMatch[2].padStart(2, '0'))
  }

  // "2024-01-15" or "2024-01"
  const isoMatch = s.match(/(\d{4})-(\d{2})(?:-(\d{2}))?/)
  if (isoMatch) {
    return _localDate(isoMatch[1], isoMatch[2], isoMatch[3] ?? '01')
  }

  // "June 23, 2026" / "Jun 23, 2026" — month name + day + year
  const mdyNameMatch = s.match(/([A-Za-z]+)\s+(\d{1,2}),?\s+(\d{4})/)
  if (mdyNameMatch) {
    const d = new Date(`${mdyNameMatch[1]} ${mdyNameMatch[2]}, ${mdyNameMatch[3]}`)
    if (!isNaN(d)) return d
  }

  // "January 2024" / "Jan 2024" — month name + year only
